...
def _rl_config_from_form(
    *,
    ...
    repetition_avoidance: str | None,
    repetition_threshold: str | None,
    eval_games: str | None,
    seed: str | None,
) -> RLConfig:
    ...
    return RLConfig(
        ...
        self_play_workers=max(1, _rl_parse_optional_int(self_play_workers) or 1),
        ...
        self_play_repetition_avoidance=max(0.0, _rl_parse_optional_float(repetition_avoidance) or 0.35),
        self_play_repetition_threshold=max(-1.0, min(1.0, _rl_parse_optional_float(repetition_threshold) or 0.20)),
...
def rl_start(
    ...
    self_play_workers: str | None = Form(None),
    run_name: str | None = Form(None),
    ...
    repetition_avoidance: str | None = Form(None),
    repetition_threshold: str | None = Form(None),
    eval_games: str | None = Form(None),
    seed: str | None = Form(None),
    save_path: str | None = Form(None),
    samples_path: str | None = Form(None),
    ...
    status = RL_HUB.start(
        config,
        preset=preset_name,
        save_path=(Path(save_path) if save_path and save_path.strip() else None),
        samples_path=(Path(samples_path) if samples_path and samples_path.strip() else None),
